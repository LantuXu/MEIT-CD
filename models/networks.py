...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif args.net_G == 'MEIT':
        net = MEIT(input_nc=3, output_nc=2, token_len=4, resnet_stages_num=4,
                               with_pos='learned', enc_depth=1, dec_depth=8)
    else:
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        self.classifier = TwoLayerConv2d(in_channels=32*4, out_channels=output_nc)#记得修改
    ...
    def forward_single(self, x):#(b,3,1024,1024)
        # resnet layers
        x_temp = self.resnet.conv1(x)#(b,64,512,512)
        x = self.resnet.bn1(x_temp)
        x = self.resnet.relu(x)
        x = self.resnet.maxpool(x)#(b,64,256,256)
        x_4 = self.resnet.layer1(x) # 1/4, in=64, out=64#(b,64,256,256)
        x_8 = self.resnet.layer2(x_4) # 1/8, in=64, out=128#(b,128,128,128)

        if self.resnet_stages_num > 3:
            x_8 = self.resnet.layer3(x_8) # 1/8, in=128, out=256#(b,256,128,128)
        if self.resnet_stages_num == 5:
            x_8 = self.resnet.layer4(x_8) # 1/32, in=256, out=512#(b,512,128,128)
        elif self.resnet_stages_num > 5:
            raise NotImplementedError

        if self.if_upsample_2x:
            x = self.upsamplex2(x_8)#(b,256,256,256)
        else:
            x = x_8#(b,256,128,128)
        # output layers
        x = self.conv_pred(x)#(b,32,256,256)/(b,32,128,128)
        # print(f"x:{x.shape},x_4:{x_4.shape},x_temp:{x_temp.shape}")
        return x
