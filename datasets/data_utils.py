...
class CDDataAugmentation:
    ...
    def __init__(
            self,
            img_size,
            with_random_hflip=False,#随机水平反转
            with_random_vflip=False,#随机垂直反转
            with_random_rot=False,#随机旋转
            with_random_crop=False,#随机裁剪
            with_scale_random_crop=False,#随机缩放裁剪
...
def pil_crop(image, box, cropsize, default_value):#裁剪
...
def get_random_crop_box(imgsize, cropsize):#裁剪
    h, w = imgsize
    ch = min(cropsize, h)
    cw = min(cropsize, w)
...
def pil_rescale(img, scale, order):#缩放
...
def pil_resize(img, size, order):#调整大小
    assert isinstance(img, Image.Image)
